...
static constexpr size_t RACEBOX_PAYLOAD_LEN = 80;  // payload is always 80 bytes
...
bool raceBoxParse(const UbxPacket& pkt, RaceBoxData& out) {
    ...
    // offset 60: Heading Accuracy (UInt32) — not stored in RaceBoxData
    // offset 64: PDOP (UInt16)             — not stored in RaceBoxData
    // offset 66: Lat/Lon Flags (1 byte)    — not stored in RaceBoxData

    // ── Battery (offset 67, RaceBox BLE Protocol rev 8) ──────────────────────
    // Mini/Mini S: bit7 = charging, bits[6..0] = battery level (0..100%)
    // Micro:       full byte = input voltage × 10 (e.g. 52 → 5.2 V)
    out.batteryLevel = p[67] & 0x7F;
    out.charging     = (p[67] & 0x80) != 0;

    // ── IMU (offsets from RaceBox BLE Protocol rev 8) ────────────────────────
    // GForce axes: X = front/back, Y = right/left, Z = up/down
    out.gForceX  = readI16(p, 68);  // mG
    out.gForceY  = readI16(p, 70);
    out.gForceZ  = readI16(p, 72);
    // RotRate axes: X = roll, Y = pitch, Z = yaw
    out.rotRateX = readI16(p, 74);  // 0.01 deg/s
    out.rotRateY = readI16(p, 76);
    out.rotRateZ = readI16(p, 78);
